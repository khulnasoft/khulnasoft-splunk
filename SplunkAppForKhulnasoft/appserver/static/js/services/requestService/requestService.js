  ...
  module.service('$requestService', [
    ...
    function ($https, $apiIndexStorageService, $q, $apiResponseModelFactory) {
      ...
      const getWellFormedUri = (endpoint) => {
        try {
          return `${getBaseUrl()}en-US/custom/SplunkAppForKhulnasoft/${endpoint}`
        } catch (err) {
      ...
      /**
       * Performs a GET request to Khulnasoft API
       * @param {String} endpoint
       * @param {Object} opts
       *
       * @see apiReqInModel
      ...
      const apiReq = async (endpoint, opts = null, method = 'GET') => {
        try {
          ...
          if (
            ...
            throw new Error('ERROR3099 - Khulnasoft not ready yet.')
          }
          return result
        } catch (err) {
      ...
      const khulnasoftIsReady = async (opts = null) => {
        try {
          $https.defaults.headers.post['Content-Type'] =
            'application/x-www-form-urlencoded'
          const currentApi = $apiIndexStorageService.getApi()
          const apiId =
            currentApi && currentApi['_key'] ? currentApi['_key'] : opts['_key']
          const endpoint = '/api/khulnasoft_ready'
          const method = 'GET'
      ...
      /**
       * Performs a request to the Khulnasoft API and returns its response in a model.
       * @param {String} endpoint
      ...
      const service = {
        ...
        khulnasoftIsReady: khulnasoftIsReady,
