/*
 * Khulnasoft app - Khulnasoft table with data as input parameter directive
  ...
  app.directive('wzDataTable', function (BASE_URL) {
    return {
      ...
      controller($scope, $filter, $notificationService, $window) {
        ...
        $scope.totalItems = 0
        $scope.khulnasoft_table_loading = true
        $scope.items = []
        /**
        ...
        const init = async () => {
          ...
          $scope.khulnasoft_table_loading = true
          await fetch()
          $scope.khulnasoft_table_loading = false
        }
        /**
      ...
      templateUrl:
        BASE_URL +
        '/static/app/SplunkAppForKhulnasoft/js/directives/wz-data-table/wz-data-table.html',
    }
  })
})
