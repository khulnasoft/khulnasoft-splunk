/*
 * Khulnasoft app - Khulnasoft table directive click wrapper
...
define([], function () {
  ...
  return function clickAction(
    ...
    if (
      ...
      new RegExp(/^\/agents\/groups\/[a-zA-Z0-9_\-.]*$/).test(instance.path) || //FIXME: This endpoints doesn't exist in the current API versions. This should be checked and removed if is not necessary
      ...
      $scope.$emit('khulnasoftShowGroup', { group: item })
    } else if (
      new RegExp(/^\/agents\/groups\/[a-zA-Z0-9_\-.]*\/files$/).test(
        //FIXME: This endpoint doesn't exist in the current API versions. This should be checked and replaced by the valid one
        // eslint-disable-line
        instance.path
      )
    ) {
      $scope.$emit('khulnasoftShowGroupFile', {
      ...
      $scope.$emit('khulnasoftShowClusterNode', { node: item })
    }
  }
})
