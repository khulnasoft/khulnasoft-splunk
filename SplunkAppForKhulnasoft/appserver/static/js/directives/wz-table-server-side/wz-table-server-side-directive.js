/*
 * Khulnasoft app - Khulnasoft table directive
...
define([
  ...
  'use strict'
  app.directive('khulnasoftTableServerSide', function (BASE_URL) {
    return {
      restrict: 'E',
      scope: {
        path: '=path',
      ...
      controller(
        ...
        const fetch = async (
          ...
          try {
            ...
            $scope.$emit('khulnasoftFetched', { items: $scope.items })
            return
          } catch (error) {
            if (
              error &&
        ...
        $scope.$on('khulnasoftUpdateInstancePath', (event, parameters) =>
          listeners.khulnasoftUpdateInstancePath(parameters, instance, init)
        )

        $scope.$on('khulnasoftFilter', (event, parameters) =>
          listeners.khulnasoftFilter(parameters, filter)
        )

        $scope.$on('khulnasoftSearch', (event, parameters) =>
          listeners.khulnasoftSearch(parameters, instance, search)
        )

        $scope.$on('khulnasoftSort', (event, parameters) =>
          $scope.sort(parameters.field)
        )

        $scope.$on('khulnasoftRemoveFilter', (event, parameters) =>
          listeners.khulnasoftRemoveFilter(
            parameters,
            instance,
            $tableFilterService,
            init
        ...
        $scope.$on('khulnasoftQuery', (event, parameters) => {
          $scope.khulnasoftTableLoading = true
          listeners.khulnasoftQuery(parameters, query)
        })

        $scope.$on('khulnasoftPlayRealTime', () => {
          realTime = true
          return realTimeFunction()
        })

        $scope.$on('khulnasoftStopRealTime', () => {
          realTime = false
          return init()
        })
      ...
      templateUrl:
        BASE_URL +
        '/static/app/SplunkAppForKhulnasoft/js/directives/wz-table-server-side/wz-table-server-side.html',
    }
  })
})
