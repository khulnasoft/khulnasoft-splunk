    ...
    return {
      controller: function ($scope, $checkDaemonsService) {
        ...
        $scope.$on('showHeadToaster', (event, data) => {
          ...
          try {
            if (!$scope.khulnasoftNotReadyYet) {
        ...
        // Listen for khulnasoft not ready event
        $scope.$on('khulnasoftNotReadyYet', (event, data) => {
          let msg = false
          $scope.showHeadToaster = false
          if (data && data.msg) {
            msg = data.msg
        ...
        // Bind function to button to launche ping to check Khulnasoft daemons again
        $scope.checkKhulnasoftAgain = () => {
      ...
      templateUrl:
        BASE_URL +
        '/static/app/SplunkAppForKhulnasoft/js/directives/wz-head-toaster/wz-head-toaster.html',
    }
  })
})
