  ...
  class Agents {
    ...
    constructor(
      ...
      try {
        ...
        const summary = this.formatAgentStatusData(
          parsedResult.agent_status.connection
        )
        const agentSynced = this.formatAgentStatusData(
          parsedResult.agent_status.configuration
        )
      ...
      this.scope.getAgentStatus = () => {
        try {
          ...
          if (nodeType === 'manager.name') {
            ...
            agentsStatusFilter = `manager.name=${managerName} index=khulnasoft-monitoring*`
          } else {
            const clusterName = filters['cluster.name']
            agentsStatusFilter = `cluster.name=${clusterName} index=khulnasoft-monitoring*`
          }
     ...
     */
    query(query, search) {
      this.scope.$broadcast('khulnasoftQuery', { query, search })
    }
