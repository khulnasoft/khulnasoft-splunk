      ...
      this.scope.$on('APP_REVISION_MISMATCH', (_event, _data) => {
        this.notificationService.showWarningToast(
          'Warning: Versions Conflict</br>' +
            "The version of the Khulnasoft App in your browser does not match the \
           App's version installed in Splunk. Please, clear your browser's cache."
        )
      })

      // Show a warning toast if the App's versions and the
      // Khulnasoft API version do not match.
      // @reads_event KHULNASOFT_VERSION_MISMATCH
      this.scope.$on('KHULNASOFT_VERSION_MISMATCH', (_event, data) => {
        this.notificationService.showWarningToast(
          'Warning: Versions Conflict</br>' +
            'The version of the Khulnasoft App does not match the Khulnasoft API version.</br>' +
            `App version: ${data.appVersion}, Khulnasoft version: ${data.APIversion}`
        )
      })
