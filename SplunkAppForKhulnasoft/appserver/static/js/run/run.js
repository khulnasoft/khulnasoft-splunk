  ...
  module.run([
    ...
    function (
      ...
      async function checkVersions() {
        ...
        // Check that the Khulnasoft API version matches the App's version.
        const appVersion = $appVersionService.getAppInfo().version
        await checkKhulnasoftVersions(appVersion)
        ...
        // Utility inner function.
        async function checkKhulnasoftVersions(appVersion) {
          try {
            ...
            if (result.data && result.data.data && !result.data.error) {
              ...
              // Show toast on versions mismatch.
              // @sends_event KHULNASOFT_VERSION_MISMATCH
              if (appVersion !== APIversion) {
                $rootScope.$broadcast('KHULNASOFT_VERSION_MISMATCH', {
                  appVersion,
                  APIversion,
                })
              }
      ...
      async function checkBeforeTransition(state) {
        try {
          ...
          $rootScope.khulnasoftNotReadyYet = false
          $rootScope.khulnasoftCouldNotBeRecovered = false
        } catch (err) {
          if (
            ...
            $rootScope.$broadcast('khulnasoftNotReadyYet', {})
      ...
      // Check secondary states when Khulnasoft is not ready to prevent
      // changing the state.
      $transitions.onBefore({}, async (trans) => {
        ...
        if (
          ...
          const khulnasoftIsReady =
            window.localStorage.getItem('khulnasoftIsReady') === 'true'
          if (!khulnasoftIsReady) {
            return false
          }
        }
      })
      ...
      // When access to a state and Khulnasoft is not ready is detected, this
