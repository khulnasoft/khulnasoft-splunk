# -*- coding: utf-8 -*-
"""
Khulnasoft app - Manager backend.
...
from khulnasoft_api import Khulnasoft_API
...
class manager(controllers.BaseController):
    ...
    def polling_state(self, **kwargs):
        ...
        try:
            ...
            app = cli.getConfStanza(
                'inputs',
                'script:///opt/splunk/etc/apps/SplunkAppForKhulnasoft/bin/get_agents_status.py'
            )
    ...
    def get_apis(self, **kwargs):
        """
        Obtain all Khulnasoft APIs from DB.
            ...
            parsed_data = jsonbak.dumps(
                {
                    ...
                    'logs_path': make_splunkhome_path(
                        ['var', 'log', 'splunk', 'SplunkAppForKhulnasoft.log'])
                }
            )
        except Exception as e:
    ...
    def check_connection(self, **kwargs):
        ...
        try:
            self.check_khulnasoft_version(api)
            # Raise error if Khulnasoft Daemons are not ready.
            ...
            if e == "DAEMONS-NOT-READY":
                self.logger.error(
                    "manager: Cannot connect to API; Khulnasoft not ready yet.")
                return jsonbak.dumps(
                    {
                        "status": 200,
                        "error": 3099,
                        "message": "Khulnasoft not ready yet."
                    }
                )
            else:
    ...
    def upload_file(self, **kwargs):
        ...
        try:
            ...
            # khulnasoft_api.py verifies the "origin" of the request to be "raw" when a binary file is sent
    ...
    def check_khulnasoft_version(self, api: API_model):
        """
        Check Khulnasoft version.

        Compares the App's Khulnasoft version with the API's Khulnasoft version.
        ...
        """
        self.logger.debug("manager::check_khulnasoft_version() called")
        ...
        try:
            ...
            if api_version != app_version:
                raise Exception(
                    "Unexpected Khulnasoft version. App version: %s, API version: %s"
                    % (app_version, api_version))
        except Exception as e:
            self.logger.error("Error checking the Khulnasoft API version: %s" % (e))
            raise e
