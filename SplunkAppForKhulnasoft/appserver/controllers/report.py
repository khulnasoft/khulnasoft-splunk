# -*- coding: utf-8 -*-
"""
Khulnasoft app - Report generation backend.
...
class PDF(FPDF):
    def header(self):
        ...
        # Logo
        self.image(
            '/opt/splunk/etc/apps/SplunkAppForKhulnasoft/appserver/static/css/images/khulnasoft/png/logo.png',
            10, 10, 65, 0)
        ...
        self.cell(0, 5, 'info@khulnasoft.com', 0, 0, 'R')
        self.ln()  # Break line
        self.cell(150)  # Move to the right
        self.cell(0, 5, 'https://khulnasoft.com', 0, 0, 'R')

    # Page footer
    def footer(self):
        # Position at 1.5 cm from bottom
        self.year = datetime.datetime.now().strftime('%Y')
        self.copyright = 'Copyright © ' + str(self.year) + ' Khulnasoft, Inc.'
        self.set_y(-15)
        self.set_text_color(37, 107, 209)
...
class report(controllers.BaseController):
    ...
    def __init__(self):
        ...
        try:
            self.logger = log()
            self.path = '/opt/splunk/etc/apps/SplunkAppForKhulnasoft/appserver/static/'
        ...
        try:
            ...
            for n in data['data']['configurations']:
                try:
                    ...
                    for currentSection in n['sections']:
                        ...
                        if 'groupConfig' in currentSection:
                            ...
                            if not conf_data or 'data' not in conf_data:
                                ...
                                self.setTableTitle(pdf)
                                # Get Khulnasoft version for the documentation link
                                ...
                                pdf.cell(
                                    ...
                                    align='C', 
                                    fill=False,
                                    link=f'https://documentation.khulnasoft.com/{docu_version}/user-manual/reference/centralized-configuration.html'
                                )
                                pdf.add_page()
                                pdf.ln(20)
                            else:
                    ...
                    self.logger.error(e)
            # Save pdf
            pdf.output(self.path+'khulnasoft-'+pdf_name+'-'+report_id+'.pdf', 'F')
            self.logger.info('report agent configuration successful' +
                             self.path+'khulnasoft-'+pdf_name+'-'+report_id+'.pdf')
        except Exception as e:
    ...
    def generate(self, **kwargs):
        ...
        try:
            ...
            # Save pdf
            pdf_final_name = 'khulnasoft-'+pdf_name+'-'+report_id+'.pdf'
