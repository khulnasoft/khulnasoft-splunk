...
def check_daemons(api: API_model) -> bool:
    ...
    execd, modulesd, khulnasoftdb and clusterd
    ...
    if 'error' in daemons_status and daemons_status['error'] > 0:
        ...
        daemons = {
            "execd": d['khulnasoft-execd'],
            "modulesd": d['khulnasoft-modulesd'],
            "db": d['khulnasoft-db']
        }
        if cc:
            daemons['clusterd'] = d['khulnasoft-clusterd']
        values = list(daemons.values())
        # Checks all the status are equals, and running
        khulnasoft_ready = len(set(daemons.values())) == 1 and values[0] == "running"        

    # Log result
    if khulnasoft_ready:
        checked_debug_msg = "Khulnasoft daemons ready"
    else:
        checked_debug_msg = "Khulnasoft daemons not ready yet"
    logger.debug(
        "check_daemons(): %s" % checked_debug_msg
    )
    return khulnasoft_ready
