...
// ------------------------------------------------------------------------- //
//  Update appVersionService.js with latest Khulnasoft version and revision number.
// ------------------------------------------------------------------------- //
function updateAppVersionServiceFile() {
  const FILE = './SplunkAppForKhulnasoft/appserver/static/js/services/app-version/appVersionService.js'
...
function updatePackageConfFile() {
  const FILE = './SplunkAppForKhulnasoft/default/package.conf'
...
// ------------------------------------------------------------------------- //
//  Update app.conf with latest Khulnasoft version and App's revision number.
// ------------------------------------------------------------------------- //
function updateAppConfFile() {
  const FILE = './SplunkAppForKhulnasoft/default/app.conf'
  ...
  let template = `\
    ...
    [launcher]
    version = ${version}
    author = info@khulnasoft.com
    description = Khulnasoft helps you to gain deeper security visibility into your infrastructure by monitoring hosts at an operating system and application level.

    [package]
    id = SplunkAppForKhulnasoft
    check_for_updates = 1
