...
/*
  Description: this script generates a file with the extrated data of Khulnasoft API 4.0, formatted to use in Khulnasoft app.

  Requirements:
    - Khulnasoft manager 4.0 up

  Use: node generate-api-info.js KHULNASOFT_API_URL [-f filename]
...
// Constants
const KHULNASOFT_API_URL = process.argv[2] // Khulnasoft API url is a required argument
...
const KHULNASOFT_DOCUMENTATION_API_REFERENCE_URL =
  'https://documentation.khulnasoft.com/current/user-manual/api/reference.html'
...
// Main method
const main = () => {
  // Check Khulnasoft API url argument is defined
  if (!KHULNASOFT_API_URL) {
    exitWithMessage('Khulnasoft API url is not defined.')
  }
  // Check Khulnasoft API url argument is valid
  if (!KHULNASOFT_API_URL.startsWith('http')) {
    exitWithMessage(
      `Khulnasoft API url is not valid. It should start with "http". Example: https://172.16.1.2:55000`
    )
  }

  // Log the configuration:
  console.log('--------------- Configuration ---------------')
  console.log(`Khulnasoft API url: ${KHULNASOFT_API_URL}`)
...
const generateAPIEndpointsInformation = async () => {
  try {
    ...
    const apiData = await request(`${KHULNASOFT_API_URL}/openapi.json`)
    // Parse response to JSON
    const jsonData = JSON.parse(apiData)
    // Extract the endpoints, mapped as { [httpMethod: ('GET' | 'PUT' | 'POST' | 'DELETE' | 'HEAD')]: endpoint[]}
    const extractedEndpoints = Object.keys(jsonData.paths).reduce(
      (accum, endpointPath) => {
    ...
    const resultEndpoints = Object.keys(extractedEndpoints).map(
      (httpMethod) => ({
        method: httpMethod,
        endpoints: extractedEndpoints[httpMethod].sort(
          sortAlphabeticalByNameProp
        ),
      })
    )
...
const generateAPISecurityActionsInformation = async () => {
  ...
  const username = 'khulnasoft'
  const password = 'khulnasoft'
  try {
    const authenticationResponse = await request(
      `${KHULNASOFT_API_URL}/security/user/authenticate`,
      {
        method: 'POST',
        headers: {
          Authorization:
    ...
    const securityActionsResponse = await request(
      `${KHULNASOFT_API_URL}/security/actions`,
      {
        headers: {
    ...
    // Save the formatted endpoints data to a file
    saveFileToOutputDir(
      OUTPUT_SECURITY_ACTIONS_FILENAME,
...
// Formatters
// Format the endpoint to use in the Khulnasoft app
const formatEndpoint = (endpointData, jsonData) => {
  ...
  return {
    ...
    ...(endpointPathParams.length
      ? {
          args: endpointPathParams
            .map(formatterEndpointParams)
            .sort(sortAlphabeticalByNameProp)
            .map((item) => ({ ...item, name: `:${item.name}` })),
        }
      : {}),
    ...(endpointQueryParams.length
      ? {
          query: endpointQueryParams
            .map(formatterEndpointParams)
            .sort(sortAlphabeticalByNameProp),
        }
      : {}),
    ...(endpointBodyParams.length
      ? {
          body: endpointBodyParams
            .map(formatterEndpointParams)
            .sort(sortAlphabeticalByNameProp),
        }
      : {}),
...
const formatEndpointFullParams = ({ in: typeParam, ...endpointData }) =>
  endpointData
const formatterEndpointParams = OUTPUT_MODE_FULL
  ? formatEndpointFullParams
  : formatEndpointSimpleParams
const formatEndpointPath = (endpointPath) =>
  endpointPath.replace(reEndpointPathArgs, (capture, group) => {
    return `:${group}`
  })
...
const createLog =
  (tag, consoleColorCode) =>
  (...args) =>
    console.log(`\x1b[${consoleColorCode}m[${tag}]\x1b[0m`, ...args)
...
const generateEndpointDocumentationLink = (endpointData) =>
  `${KHULNASOFT_DOCUMENTATION_API_REFERENCE_URL}#operation/${endpointData.operationId}`
...
const saveFileToOutputDir = (filename, content) => {
  ...
  fs.writeFile(filePath, content, function (error, data) {
    if (error) {
      return logger.error(
        `An error appeared saving the output file "${filePath}":`,
        error
      )
    }
